...
auto all_ = [](auto &&... sf) {
  return [=](auto &&... xs) mutable {
    return hana::make_tuple (
      sf (std::forward<typename std::decay<decltype (xs)>::type> (xs)...)...);
  };
};
...
auto both_ = [](auto &&f1, auto &&f2) {
  return [=](auto &&... xs) {  //xs are Product types
    return hana::make_pair (f1 (hana::first (xs)...),
                            f2 (hana::second (xs)...));
  };
};
...
/// make a pair
auto pair_ = [](auto &&a, auto &&b) { return hana::make_pair (a, b); };
...
auto mergeall_ = [](auto &&f) {
  return [=](auto &&... xs) mutable { return f (FORWARD (xs)...); };
};
...
namespace hana {
  ...
  // can now take any number of arguments, even 0
...
struct _pipe
{
  ...
  constexpr decltype (auto) operator() (Xs &&... xs) &&
  {
    return std::move (g) (std::move (f) (static_cast<Xs &&> (xs)...));
  }
};
...
struct _split
{
  ...
  constexpr decltype (auto) operator() (Xs &&... xs) const &
  {
    return make_pair (f (static_cast<Xs &&> (xs)...),
                      g (static_cast<Xs &&> (xs)...));
  }
  ...
  constexpr decltype (auto) operator() (Xs &&... xs) &&
  {
    return make_pair (std::move (f) (static_cast<Xs &&> (xs)...),
                      std::move (g) (static_cast<Xs &&> (xs)...));
  }
};
