...
/// @file
///
/// @brief Signals are functions of time in milliseconds -- to be fed time and return anything)
///
...
/// @todo develop notation for distinguishing between signals and signalfunctions
  ...
  /// Return Input (e.g bypass)
  auto id_ = [](auto&& x) { return x; };

  /// lift true onto signal true_: true_(t) -> true
  auto true_ = [](auto&& x){ return true; };

  /// switch once takes a signalfunction sf1 that takes a t and returns
  /// a pair (first:f(t), second:event(t))
  /// a signalfunction sf2 that takes the result of event(t) and returns
  /// a signal f(t)
  ...
  /// if signal is true, return result
  auto if_ = [](auto&& x){
    return [=](auto&& e){
        using T = TYPE(e);
        if (e) return maybe<T>(x);
        return maybe<T>();
    };
  };
...
auto impulse_ = [](float&& x){
  ...
  return hana::compose( trigger_on_reset_(), mod_(1000.0/static_cast<float&&>(x)) );
};
...
auto linear_ = [](float&& x){
   return switch_once_(
               ...
               hana::pipe_( reset_(), ramp_(1.0/x), hana::split_( id_, trigger_on_reset_() ) ),
